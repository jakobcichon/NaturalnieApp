...
namespace NaturalnieApp.Forms
{
    public partial class SupplierList : UserControl
    {
        ...
        DataSourceRelated.SupplierColumnNames ColumnNames;
        ...
        public SupplierList()
        {
            ...
            this.ColumnNames = new DataSourceRelated.SupplierColumnNames();
            ...
            this.lName.Text = "Lista dostawców";
        }
        #endregion
        //=============================================================================
        //                              Background worker
            ...
            try
            {
                ...
                switch (taskType)
                {
                    case backgroundWorkerTasks.Init:
                        ...
                        {
                            List<Supplier> productSupplierList = this.databaseCommands.GetAllSupplierEnts();

                            //Convert to data row
                            foreach (Supplier supplier in productSupplierList)
                            {
                                DataRow dataRow = returnDataTable.NewRow();
                                dataRow.SetField<int>(this.ColumnNames.Id, supplier.Id);
                                dataRow.SetField<string>(this.ColumnNames.Name, supplier.Name);
                                dataRow.SetField<string>(this.ColumnNames.Info, supplier.Info);
        ...
        void SaveManufacturerTableChangestoDB(DataTable dtDifferences, DataTable dtAdded, DataTable dtRemoved)
        {
            ...
            {
                string supplierName = row.Field<string>(this.ColumnNames.Name);

                //Get Manufacturer product count
                List<string> productsList = this.databaseCommands.GetProductsNameListBySupplier(supplierName);
                ...
                if(productsList.Count() > 0)
                {
                    string message = string.Format("Uwaga! W bazie danych istnieją produkty związane z producentem {0}. " +
                        "Najpierw należy usunąć produkty powiązane z tym producentem.", supplierName);
                ...
                {
                    this.databaseCommands.DeleteSupplier(row[this.ColumnNames.Name].ToString());
                }
            ...
            {
                //Get original entity
                Supplier localEntity = this.databaseCommands.GetSupplierEntityById(Convert.ToInt32(row[this.ColumnNames.Id]));

                //Override with new values
                localEntity.Name = row.Field<string>(this.ColumnNames.Name);
                localEntity.Info = row.Field<string>(this.ColumnNames.Info);

                this.databaseCommands.EditSupplier(localEntity);
            }
            ...
            {
                //Local entity
                Supplier localEntity = new Supplier();
                
                //Write values
                localEntity.Name = row.Field<string>(this.ColumnNames.Name);
                localEntity.Info = row.Field<string>(this.ColumnNames.Info);

                this.databaseCommands.AddSupplier(localEntity);
            }
